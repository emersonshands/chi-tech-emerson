...
chi_math::ProductQuadratureOp::ProductQuadratureOp(const chi_math::ProductQuadrature&
 inquad, const int inmethod, const int order) :
 quad(inquad), method(inmethod), sn(order)
{
...
void chi_math::ProductQuadratureOp::MakeHarmonicIndices()
{
  ...
  const int Lmax = 2 * (static_cast<int>(sn) - 1);
  for (int ell = 0; ell <= Lmax; ++ell)
    for (int m = 0; m <= ell; m += 1)
    {
//      printf("Current m %i, current l %i\n", m, ell);
      if (ell >= nquad)
      {
